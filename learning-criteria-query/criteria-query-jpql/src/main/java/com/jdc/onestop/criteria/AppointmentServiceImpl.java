...
public class AppointmentServiceImpl extends AbstractService implements AppointmentService {
	...
	public List<AppointmentListItem> search(AppointmentSearch search) {
		try (var em = emSupplier.get()) {
			// select a.doctor.id, a.doctor.name, a.patient.id, a.patient.name, a.patient.contact.phone, a.id.appointmentDate, a.id.startTime, a.seqNumber, a.registAt from Appointment a where 1 = 1
			
			var params = new HashMap<String, Object>();
			var sb = new StringBuffer();
			
			sb.append("""
					select new com.jdc.onestop.criteria.output.AppointmentListItem(
					a.doctor.id, a.doctor.name, a.patient.id, a.patient.name, a.patient.contact.phone, 
					a.id.appointmentDate, a.id.startTime, a.seqNumber, a.registAt) 
					from Appointment a where 1 = 1
					""");
			
			if (!isBlank(search.doctorName())) {
				sb.append(" and lower(a.doctor.name) like :dname");
				params.put("dname", search.doctorName().toLowerCase().concat("%"));
			}
			
			if (!isBlank(search.patientName())) {
				sb.append(" and lower(a.patient.name) like :pname");
				params.put("pname", search.patientName().toLowerCase().concat("%"));
			}

			if (!isBlank(search.patientPhone())) {
				sb.append(" and lower(a.patient.contact.phone) like :phone");
				params.put("phone", search.patientPhone().toLowerCase().concat("%"));
			}
			
			if (!isBlank(search.startTime())) {
				sb.append(" and a.id.startTime = :startTime");
				params.put("startTime", search.startTime());
			}
			
			if (null != search.from()) {
				sb.append(" and a.id.appointmentDate >= :from");
				params.put("from", search.from());
			}
			
			if (null != search.to()) {
				sb.append(" and a.id.appointmentDate <= :to");
				params.put("to", search.to());
			}
			
			if (null != search.canceled()) {
				sb.append(" and a.cancled = :cancel");
				params.put("cancel", search.canceled());
			}
			
			var query = em.createQuery(sb.toString(), AppointmentListItem.class);
