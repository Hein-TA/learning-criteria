...
import com.jdc.onestop.criteria.embeddable.AppointmentPk_;
...
import com.jdc.onestop.criteria.entity.Appointment_;
...
public class AppointmentServiceImpl extends AbstractService implements AppointmentService {
	...
	public List<AppointmentListItem> search(AppointmentSearch search) {
		try (var em = emSupplier.get()) {
			...
			cq.multiselect(
					root.get(Appointment_.doctor).get(Doctor_.id),
					root.get(Appointment_.doctor).get(Doctor_.name),
					root.get(Appointment_.patient).get(Patient_.id),
					root.get(Appointment_.patient).get(Patient_.name),
					root.get(Appointment_.patient).get(Patient_.contact).get(Contact_.phone),
					root.get(Appointment_.id).get(AppointmentPk_.appointmentDate),
					root.get(Appointment_.id).get(AppointmentPk_.startTime),
					root.get(Appointment_.seqNumber),
					root.get(Appointment_.registAt)
					);
			
			var params = new ArrayList<Predicate>();
			
			if (!isBlank(search.doctorName())) {
				params.add(cb.like(cb.lower(root.get(Appointment_.doctor).get(Doctor_.name)), search.doctorName().toLowerCase().concat("%")));
			}

			if (!isBlank(search.patientName())) {
				params.add(cb.like(cb.lower(root.get(Appointment_.patient).get(Patient_.name)), search.patientName().toLowerCase().concat("%")));
			}

			if (!isBlank(search.patientPhone())) {
				params.add(cb.like(cb.lower(root.get(Appointment_.patient).get(Patient_.contact).get(Contact_.phone)), search.patientPhone().toLowerCase().concat("%")));
			}
			
			if (!isBlank(search.startTime())) {
				params.add(cb.equal(root.get(Appointment_.id).get(AppointmentPk_.startTime), search.startTime()));
			}
			
			if (null != search.from()) {
				params.add(cb.greaterThanOrEqualTo(root.get(Appointment_.id).get(AppointmentPk_.appointmentDate), search.from()));
			}
			
			if (null != search.to()) {
				params.add(cb.lessThanOrEqualTo(root.get(Appointment_.id).get(AppointmentPk_.appointmentDate), search.to()));
			}
			
			if (null != search.canceled()) {
				params.add(cb.equal(root.get(Appointment_.cancled), search.canceled()));
			}
